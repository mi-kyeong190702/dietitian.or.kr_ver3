<%@LANGUAGE="VBSCRIPT" CODEPAGE="949"%>

<!--#include virtual="/inc/Mass_InjectionFunction.asp"-->
<!--#include virtual="/inc/Mass_InjectionAction.asp"-->

<!--#include virtual="/db/dbopen.inc" -->
<!--#include virtual="/inc/_function.asp" -->

<%
	function word(Str)		'따옴표 제거
		if xup.IsItem(Str) = 1 then
			Str = xup(Str)
			Str = replace(Str, "'", "''")
			word = Str
		else
			word = ""
		end if
	End function


	Server.ScriptTimeout = 3000
	set xup = Server.CreateObject("UpDownExpress.FileUpload")
	xup.InitControl
	boardcode = word("boardcode")




%>
<!--#include virtual="/board/_BoardSettings.asp" -->
<%

	

	if Bs_Write = false then
		set xup = nothing
		error("글 작성권한이 없습니다.")
	end if

	ListUrl = word("ListUrl")



	function notag(Str)
		Str = replace(Str, "<","<X-")
		notag = Str
	end function

	Function notag2(Str)
		Str = replace(Str, "&lt;", "&lT;")
		Str = replace(Str,"<X-","&lt;")
		notag2 = Str
	end Function

	function lifetag(Str)
		Tag_temp = "a font center p b i u h img t div strong !-- embed"
		Tag_u = split(ucase(Tag_temp)," ")
		Tag_l = split(lcase(Tag_temp)," ")
		for i = 0 to ubound(Tag_u)
			Str = replace(Str,"<X-"&Tag_u(i),"<"&Tag_u(i))
			Str = replace(Str,"<X-"&Tag_l(i),"<"&Tag_l(i))
			Str = replace(Str,"<X-/"&Tag_u(i),"</"&Tag_u(i))
			Str = replace(Str,"<X-/"&Tag_l(i),"</"&Tag_l(i))
		next
		lifetag = Str
	end function




'	idx				= word("idx")
'	bbs_Pwd		= word("bbs_pwd")
'	bbs_name		= word("bbs_name")
'	bbs_email		= word("bbs_email")
'	bbs_category	= word("bbs_category")
'	bbs_option		= word("bbs_option")
'	bbs_subject	= word("bbs_subject")
'	bbs_contents	= word("bbs_contents")
'
'
'	bbs_subject	= notag(bbs_subject)
'	bbs_contents	= notag(bbs_contents)
'	bbs_user_ip	= Request.ServerVariables("REMOTE_ADDR")
'	dietitian_ssn	= word("dietitian_ssn")
'
'	page				= word("page")
'

	idx				= word("idx")
	bbs_Pwd		= CompareInput(word("bbs_pwd"))
	bbs_name		= CompareInput(word("bbs_name"))
	bbs_email		= CompareInput(word("bbs_email"))
	bbs_category	= CompareInput(word("bbs_category"))
	bbs_option		= CompareInput(word("bbs_option"))
	bbs_subject	= CompareInput(word("bbs_subject"))
	'bbs_contents	= CompareInput(word("bbs_contents"))
	bbs_contents	= word("bbs_contents")


	bbs_subject	= notag(bbs_subject)
	bbs_contents	= notag(bbs_contents)
	bbs_user_ip	= Request.ServerVariables("REMOTE_ADDR")
	dietitian_ssn	= word("dietitian_ssn")

	page				= word("page")

	if word("bbs_notice") = "1" then
		bbs_notice = 1
	else
		bbs_notice = 0
	end if



If boardcode ="20" Then


	 f_NotWord(CompareInput(word("bbs_name")))
	 f_NotWord(CompareInput(word("bbs_email")))
	 f_NotWord(CompareInput(word("bbs_category")))

	 f_NotWord(CompareInput(word("bbs_subject")))
	 f_NotWord(CompareInput(word("bbs_contents")))





End If
'/*-------------------- 게시판 금지어 --------------*/





			sub f_NotWord(notword_string)

				  boardNOTWOrd  = "요요뱅크|대출|담보|인증|초고속|판매|자신감|파라오|맞고|사무보조|초강력|유저|채팅|넷마블|재태크|머니|유망자격증|섹파|Sex|섹스|섹.|즉석만남|조건만남|파트너|무료|Leonoid|바카라|카지노|비아그라|몰래|원피스|변태|성인|엘리베이터|음란|부업|위조|로또|바댜이야기|뱌댜|뱌다|므료|공짜|고래|상어|Djlgtxxpmohl|Yjygzylgbovc|Gjjzyqjkqjpa|Sgpdnjwuxvgo|Nqlgkmehtgip|Zqcjbxrugsqs|Gccqnnvpyfig|Rjpqlzoqwovf|Fboaefojqkia|Kmxgchnihkle|Ymnhnjfiqrfr|Axzluipdztvd|www.las333.com|http://korea9.lu.to|http://korea10.lu.to|wkdrlvnd69|don.or.kr|http://xman.co.to|www.coco77.com|bbaa.kp.st|www.qqq11.kor.st|www.tvn8989.com|www.tvn8989.com|www.Samojoa.net|중국|민박|한방|섹|미녀|Berkeley|Viagra|p2p|gogofile|scala|야마토|바다이야기|위엄|Albahouse|hm5356|살림살이|게임|대포업|대박|허락|xy.to|사기|339race|aha.ne.kr|아이템|투어헌트|Info|허니문|폰팅|명품|도배|소문|저렴|레알|wo.to|뚱보|공회전|노하우|트랜스젠더|Yatom|Wbwnmbnbyoxk|배팅|알바|토토|재택|2jobok|대행|go28|뽕|vita88|kingwangzzang|pokasi|ghb82|GHB|바다|Ocmeca|비번|Viakorea|불리기|ez.to|고화질|진조로|호기심|쉐어|유머|프로리|찬영|추천|서핑|공유|딜러|경품|강력|작전|만냥|파라오|임성관|경마|으뜸|Taba|Auction|Otoo|c02|bbtan|disk|file|co25|xo1|gample|blog24|see.ccoo.kr|bomul|wowdisk|sm.to|9988.kr.gs|uy.to|gg.gg|sm.am|kr.gd|yssaxpfngcdm|nunawa|doumi892|の|sm.to|Race"





				 ' f_NotWord = false

				  notword_filter_arr = split(boardNOTWOrd,"|")
				  notword_filter_cnt = Ubound(notword_filter_arr)

				  for j = 0 to notword_filter_cnt

				   if InStr(1,notword_string,notword_filter_arr(j),1) > 0 Then

				'	f_NotWord = true

					error("본 게시판과 맞지 않는 단어 입력으로 해당 글을 등록하실 수 없습니다. 문의사항은 협회 사무국으로 연락을 부탁 드립니다.")
					Response.End

					exit for
				   end if
				  next

			End sub





 '/*-------------------- 게시판 금지어 --------------*/


	if bbs_name = "" then
		bbs_name = User_name
	end if


	if word("bbs_tag") = "1" then
		if Bs_sTag then
			bbs_subject = lifetag(bbs_subject)
		end if
		if Bs_cTag then
			bbs_contents = lifetag(bbs_contents)
		end if
	else
		bbs_subject = notag2(bbs_subject)
		bbs_contents = notag2(bbs_contents)
	end if


	if trim(bbs_subject) = "" Then
		error("제목을 입력해 주시기바랍니다..")
		'bbs_subject = "- 제목이 없는 글입니다. -"
	end if



	mode = word("mode")
	if mode = "new" or mode = "" then
		set rs = server.CreateObject("adodb.recordset")
		sql = "select min(bbs_m_number) from "& Bs_TableName
		rs.open sql,db,0
		bbs_m_number_t = int(rs(0))
		bbs_m_number = bbs_m_number_t - 1
		bbs_step = 1
		bbs_re_number = 0
		rs.close
		set rs = nothing
	elseif mode = "reply" and idx <> "" then
		set rs = server.CreateObject("adodb.recordset")
		sql = "select bbs_m_number, bbs_step from "& Bs_TableName &" where bbs_idx = "& idx
		rs.open sql,db,0

		bbs_m_number_t = rs("bbs_m_number")
		bbs_m_number = bbs_m_number_t + 0.01
		bbs_step = rs("bbs_step") + 1
		bbs_re_number = idx

		sql = "update "& Bs_TableName &" set bbs_m_number = bbs_m_number + 0.01 where bbs_m_number > "& rs("bbs_m_number") &" and bbs_m_number < "& int(rs("bbs_m_number")) + 1
		db.execute(sql)

		sql = "update "& Bs_TableName &" set bbs_re_count = bbs_re_count + 1 where bbs_idx= "& idx
		db.execute(sql)

		rs.close
		set rs = nothing
	elseif mode = "modify" and idx <> "" then
		set rs = server.CreateObject("adodb.recordset")
		sql = "select bbs_user_id, bbs_file_name, bbs_file_url, bbs_file_type, bbs_file_size, bbs_file_down, bbs_pwd from "& Bs_TableName &" where bbs_idx = "& idx
		rs.open sql,db,0

		if rs.eof then
			rs.close
			set rs = nothing
			set xup = nothing
			error("잘못된 호출")
		end if



		pwd = word("bbs_pwd")

		if (rs("bbs_user_id") = "" or rs("bbs_user_id") <> user_id) and UserLevel > 2 then
			if pwd = "" or rs("bbs_pwd") = "" or pwd <> rs("bbs_pwd") then
				rs.close
				set rs = nothing
				set xup = nothing
				error("글 수정 권한이 없습니다.")
			end if
		end if


		file_name	= rs("bbs_file_name")
		file_url		= rs("bbs_file_url")
		file_type		= rs("bbs_file_type")
		file_size		= rs("bbs_file_size")


		if xup.IsItem("delfile") = 1 then
			if trim(file_url) <> "" then
				if xup.IsFileExist(file_url) = true then
					xup.DeleteFile(file_url)
				end if
				file_name	= ""
				file_url		= ""
				file_type		= ""
				file_size		= 0
				file_down	= 0
			end if

			if trim(file_url2) <> "" then
				if xup.IsFileExist(file_url2) = true then
					xup.DeleteFile(file_url2)
				end if
				file_name2	= ""
				file_url2		= ""
				file_type2	= ""
				file_size2	= 0
				file_down2	= 0
			end if

			if trim(file_url3) <> "" then
				if xup.IsFileExist(file_url3) = true then
					xup.DeleteFile(file_url3)
				end if
				file_name3	= ""
				file_url3		= ""
				file_type3	= ""
				file_size3	= 0
				file_down3	= 0
			end if

'' 파일첨부 5개로 늘림 KSH
			if trim(file_url4) <> "" then
				if xup.IsFileExist(file_url4) = true then
					xup.DeleteFile(file_url4)
				end if
				file_name4	= ""
				file_url4		= ""
				file_type4	= ""
				file_size4	= 0
				file_down4	= 0
			end if

			if trim(file_url5) <> "" then
				if xup.IsFileExist(file_url5) = true then
					xup.DeleteFile(file_url5)
				end if
				file_name5	= ""
				file_url5		= ""
				file_type5	= ""
				file_size5	= 0
				file_down5	= 0
			end if
'' 파일첨부 5개로 늘림 KSH

		end if

		rs.close
		set rs = nothing
	else
		set xup = nothing
		db.close
		set db = nothing
		response.Redirect("list.asp?boardcode="& boardcode )
		response.End()
	end if


	nowTime = year(now)&GetNumber(month(now))&GetNumber(day(now))&GetNumber(hour(now))&GetNumber(minute(now))&GetNumber(second(now)) 	' 현재시간..

	up_dir = "/board/file/"& boardcode &"/"


	set fs = Server.CreateObject("Scripting.FileSystemObject")
	if fs.FolderExists(Server.MapPath(up_dir)) = false then
		fs.CreateFolder(Server.MapPath(up_dir))
	end if
	set fs = nothing

	'if xup.IsFileExist(up_dir) = false then
	'	xup.MakeDir(up_dir)
	'end if

	if xup.IsItem("file_name") = 1 then

		'파일은 있는데 수정일 경우
		if mode = "modify" then
			if trim(file_url) <> "" then
				if xup.IsFileExist(file_url) = true then
					xup.DeleteFile(file_url)
				end if
				file_name = ""
				file_url = ""
				file_type = ""
				file_size = 0
				file_down = 0
			end if
		end if


		if Bs_Pds = false then
			set xup = nothing
			error("현재 게시판은 자료실이 아닙니다. \n\n 파일 업로드를 하실수 없습니다.")
		end if


		file_name		= lcase(xup.Attachments("file_name").FileName)
		file_size			= xup.Attachments("file_name").size

		file_type0		= split(file_name,".")
		file_type			= file_type0(ubound(file_type0))
		file_life_type	= split("jpg,jpeg,gif,bmp,png,swf,tiff,tif,mpg,mpeg,avi,asf,wma,wmv,rm,ra,mp3",",")
		fileFlag			= "F"

		for i = 0 to ubound(file_life_type)
			if file_type = file_life_type(i) then
				fileFlag = "T"
			end if
		next

		if fileFlag = "T" then
			file_url = up_dir & session.SessionID & nowTime & "." & file_type
		else
			file_url = up_dir & session.SessionID & nowTime
		end if

		' 1번 파일 저장
		xup.Attachments("file_name").SaveFile file_url

	else

		if mode <> "modify" then
			file_name = ""
			file_url = ""
			file_type = ""
			file_size = 0
		end if
	end if

'	두번째 파일 업로드

	if xup.IsItem("file_name2") = 1 then
		if mode = "modify" then
			if trim(file_url2) <> "" then
				if xup.IsFileExist(file_url2) = true then
					xup.DeleteFile(file_url2)
				end if
				file_name2 = ""
				file_url2 = ""
				file_type2 = ""
				file_size2 = 0
				file_down2 = 0
			end if
		end if

		if Bs_Pds = false then
			set xup = nothing
			error("현재 게시판은 자료실이 아닙니다. \n\n 파일 업로드를 하실수 없습니다.")
		end if

		file_name2		= lcase(xup.Attachments("file_name2").FileName)
		file_size2		= xup.Attachments("file_name2").size

		file_type0		= split(file_name2,".")
		file_type2		= file_type0(ubound(file_type0))
		file_life_type2	= split("jpg,jpeg,gif,bmp,png,swf,tiff,tif,mpg,mpeg,avi,asf,wma,wmv,rm,ra,mp3",",")
		fileFlag2			= "F"

		for i = 0 to ubound(file_life_type2)
			if file_type2 = file_life_type2(i) then
				fileFlag2 = "T"
			end if
		next

		if fileFlag2 = "T" then
			file_url2 = up_dir & session.SessionID & nowTime & "_02." & file_type2
		else
			file_url2 = up_dir & session.SessionID & nowTime& "_02"
		end if

		'2번 파일 저장
		xup.Attachments("file_name2").SaveFile file_url2



	else
		if mode <> "modify" then
			file_name2 = ""
			file_url2 = ""
			file_type2 = ""
			file_size2 = 0
		end if
	end if


	'	세번째 파일 업로드

	if xup.IsItem("file_name3") = 1 then
		if mode = "modify" then
			if trim(file_url3) <> "" then
				if xup.IsFileExist(file_url3) = true then
					xup.DeleteFile(file_url3)
				end if
				file_name3 = ""
				file_url3 = ""
				file_type3 = ""
				file_size3 = 0
				file_down3 = 0
			end if
		end if

		if Bs_Pds = false then
			set xup = nothing
			error("현재 게시판은 자료실이 아닙니다. \n\n 파일 업로드를 하실수 없습니다.")
		end if

		file_name3		= lcase(xup.Attachments("file_name3").FileName)
		file_size3		= xup.Attachments("file_name3").size

		file_type0		= split(file_name3,".")
		file_type3		= file_type0(ubound(file_type0))
		file_life_type3	= split("jpg,jpeg,gif,bmp,png,swf,tiff,tif,mpg,mpeg,avi,asf,wma,wmv,rm,ra,mp3",",")
		fileFlag3			= "F"

		for i = 0 to ubound(file_life_type3)
			if file_type3 = file_life_type3(i) then
				fileFlag3 = "T"
			end if
		next

		if fileFlag3 = "T" then
			file_url3 = up_dir & session.SessionID & nowTime & "_03." & file_type3
		else
			file_url3 = up_dir & session.SessionID & nowTime & "_03"
		end if

		'3번 파일 저장
		xup.Attachments("file_name3").SaveFile file_url3



	else
		if mode <> "modify" then
			file_name3 = ""
			file_url3 = ""
			file_type3 = ""
			file_size3 = 0
		end if
	end If

'	네번째 파일 업로드 추가  KSH

	if xup.IsItem("file_name4") = 1 then
		if mode = "modify" then
			if trim(file_url4) <> "" then
				if xup.IsFileExist(file_url4) = true then
					xup.DeleteFile(file_url4)
				end if
				file_name4 = ""
				file_url4 = ""
				file_type4 = ""
				file_size4 = 0
				file_down4 = 0
			end if
		end if

		if Bs_Pds = false then
			set xup = nothing
			error("현재 게시판은 자료실이 아닙니다. \n\n 파일 업로드를 하실수 없습니다.")
		end if

		file_name4		= lcase(xup.Attachments("file_name4").FileName)
		file_size4		= xup.Attachments("file_name4").size

		file_type0		= split(file_name4,".")
		file_type4		= file_type0(ubound(file_type0))
		file_life_type4	= split("jpg,jpeg,gif,bmp,png,swf,tiff,tif,mpg,mpeg,avi,asf,wma,wmv,rm,ra,mp3",",")
		fileFlag4			= "F"

		for i = 0 to ubound(file_life_type4)
			if file_type4 = file_life_type4(i) then
				fileFlag4 = "T"
			end if
		next

		if fileFlag4 = "T" then
			file_url4 = up_dir & session.SessionID & nowTime & "_04." & file_type4
		else
			file_url4 = up_dir & session.SessionID & nowTime& "_04"
		end if

		'4번 파일 저장
		xup.Attachments("file_name4").SaveFile file_url4



	else
		if mode <> "modify" then
			file_name4 = ""
			file_url4 = ""
			file_type4 = ""
			file_size4 = 0
		end if
	end if

'	디섯번째 파일 업로드 추가  KSH

	if xup.IsItem("file_name5") = 1 then
		if mode = "modify" then
			if trim(file_url5) <> "" then
				if xup.IsFileExist(file_url5) = true then
					xup.DeleteFile(file_url5)
				end if
				file_name5 = ""
				file_url5 = ""
				file_type5 = ""
				file_size5 = 0
				file_down5 = 0
			end if
		end if

		if Bs_Pds = false then
			set xup = nothing
			error("현재 게시판은 자료실이 아닙니다. \n\n 파일 업로드를 하실수 없습니다.")
		end if

		file_name5		= lcase(xup.Attachments("file_name5").FileName)
		file_size5		= xup.Attachments("file_name5").size

		file_type0		= split(file_name5,".")
		file_type5		= file_type0(ubound(file_type0))
		file_life_type5	= split("jpg,jpeg,gif,bmp,png,swf,tiff,tif,mpg,mpeg,avi,asf,wma,wmv,rm,ra,mp3",",")
		fileFlag5			= "F"

		for i = 0 to ubound(file_life_type5)
			if file_type5 = file_life_type5(i) then
				fileFlag5 = "T"
			end if
		next

		if fileFlag5 = "T" then
			file_url5 = up_dir & session.SessionID & nowTime & "_05." & file_type5
		else
			file_url5 = up_dir & session.SessionID & nowTime& "_05"
		end if

		'5번 파일 저장
		xup.Attachments("file_name5").SaveFile file_url5



	else
		if mode <> "modify" then
			file_name5 = ""
			file_url5 = ""
			file_type5 = ""
			file_size5 = 0
		end if
	end If

'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''


	'response.write xup.Attachments.Count & "<BR>"
	For i = 1 to xup.Attachments.Count
  		'SaveFile file_url & xup.Attachments(i).FileName
		'response.write xup.Attachments(i).FileName & "<BR>"
	Next

	'response.write "file_url : " & file_url & "<br>" & "file_url2 : " & file_url2 & "<br>" &  "file_url3 : " & file_url3 & "<br>"



	'response.end


	if mode <> "modify" then

		sql = "insert into "& Bs_TableName &"("
		sql = sql & "bbs_user_id, "
		sql = sql & "bbs_board_code, "
		If bbs_notice = 1 Then
			sql = sql & "bbs_notice, "
		End If
		sql = sql & "bbs_m_number, "
		sql = sql & "bbs_step, "

		sql = sql & "bbs_file_name, "
		sql = sql & "bbs_file_url, "
		sql = sql & "bbs_file_type, "
		sql = sql & "bbs_file_size, "

		sql = sql & "bbs_file_name2, "
		sql = sql & "bbs_file_url2, "
		sql = sql & "bbs_file_type2, "
		sql = sql & "bbs_file_size2, "

		sql = sql & "bbs_file_name3, "
		sql = sql & "bbs_file_url3, "
		sql = sql & "bbs_file_type3, "
		sql = sql & "bbs_file_size3, "

		sql = sql & "bbs_file_name4, "
		sql = sql & "bbs_file_url4, "
		sql = sql & "bbs_file_type4, "
		sql = sql & "bbs_file_size4, "

		sql = sql & "bbs_file_name5, "
		sql = sql & "bbs_file_url5, "
		sql = sql & "bbs_file_type5, "
		sql = sql & "bbs_file_size5, "

		sql = sql & "bbs_pwd, "
		sql = sql & "bbs_category, "
		sql = sql & "bbs_name, "
		sql = sql & "bbs_email, "
		sql = sql & "bbs_option, "
		sql = sql & "bbs_subject, "
		sql = sql & "bbs_contents, "
		sql = sql & "bbs_user_ip, "
		sql = sql & "bbs_re_number"
		sql = sql & ") values('"
		sql = sql & user_id				&"', '"
		sql = sql & boardcode			&"', '"
		If bbs_notice = 1 Then
			sql = sql & bbs_notice			&"', '"
		End If
		sql = sql & bbs_m_number		&"', '"
		sql = sql & bbs_step			&"', '"

		sql = sql & file_name			&"', '"
		sql = sql & file_url			&"', '"
		sql = sql & file_type			&"', '"
		sql = sql & file_size			&"', '"

		sql = sql & file_name2			&"', '"
		sql = sql & file_url2			&"', '"
		sql = sql & file_type2			&"', '"
		sql = sql & file_size2			&"', '"

		sql = sql & file_name3			&"', '"
		sql = sql & file_url3			&"', '"
		sql = sql & file_type3			&"', '"
		sql = sql & file_size3			&"', '"

		sql = sql & file_name4			&"', '"
		sql = sql & file_url4			&"', '"
		sql = sql & file_type4			&"', '"
		sql = sql & file_size4			&"', '"

		sql = sql & file_name5			&"', '"
		sql = sql & file_url5			&"', '"
		sql = sql & file_type5			&"', '"
		sql = sql & file_size5			&"', '"


		sql = sql & bbs_pwd			&"', '"
		sql = sql & bbs_category		&"', '"
		sql = sql & bbs_name			&"', '"
		sql = sql & bbs_email			&"', '"
		sql = sql & bbs_option			&"', '"
		sql = sql & bbs_subject		&"', '"
		sql = sql & bbs_contents		&"', '"
		sql = sql & bbs_user_ip		&"', '"
		sql = sql & bbs_re_number	&"')"
		db.execute(sql)

	else

		sql = "update "& Bs_TableName &" set "
		sql = sql & "bbs_notice = '"		& bbs_notice		&"', "

		sql = sql & "bbs_file_name = '"		& file_name		&"', "
		sql = sql & "bbs_file_url = '"			& file_url			&"', "
		sql = sql & "bbs_file_type = '"		& file_type			&"', "
		sql = sql & "bbs_file_size = '"		& file_size			&"', "

		sql = sql & "bbs_file_name2 = '"	& file_name2		&"', "
		sql = sql & "bbs_file_url2 = '"		& file_url2			&"', "
		sql = sql & "bbs_file_type2 = '"		& file_type2			&"', "
		sql = sql & "bbs_file_size2 = '"		& file_size2		&"', "

		sql = sql & "bbs_file_name3 = '"	& file_name3		&"', "
		sql = sql & "bbs_file_url3 = '"		& file_url3			&"', "
		sql = sql & "bbs_file_type3 = '"		& file_type3			&"', "
		sql = sql & "bbs_file_size3 = '"		& file_size3		&"', "

		sql = sql & "bbs_file_name4 = '"	& file_name4		&"', "
		sql = sql & "bbs_file_url4 = '"		& file_url4			&"', "
		sql = sql & "bbs_file_type4 = '"		& file_type4			&"', "
		sql = sql & "bbs_file_size4 = '"		& file_size4		&"', "

		sql = sql & "bbs_file_name5 = '"	& file_name5		&"', "
		sql = sql & "bbs_file_url5 = '"		& file_url5			&"', "
		sql = sql & "bbs_file_type5 = '"		& file_type5			&"', "
		sql = sql & "bbs_file_size5 = '"		& file_size5		&"', "

		sql = sql & "bbs_file_down = '"		& file_down		&"', "
		sql = sql & "bbs_pwd = '"				& bbs_pwd			&"', "
		sql = sql & "bbs_category = '"		& bbs_category	&"', "
		sql = sql & "bbs_name = '"			& bbs_name		&"', "
		sql = sql & "bbs_email = '"			& bbs_email		&"', "
		sql = sql & "bbs_option = '"			& bbs_option		&"', "
		sql = sql & "bbs_subject = '"		& bbs_subject	&"', "
		sql = sql & "bbs_contents = '"		& bbs_contents	&"', "
		sql = sql & "bbs_edit_date_dt = getdate() ,"
		sql = sql & "bbs_user_ip = '"		& bbs_user_ip	&"' "
		sql = sql & " where bbs_idx = '"	& idx					&"'"
		db.execute(sql)

		'response.write sql
		'response.end
	end if

	set xup = nothing
	db.close
	set db = nothing
	'response.end


'----------------------------- 글쓰기 일 때 포인트 증가 시키는 부분 ---------------------------
	user_id=Request.Cookies("dietitian")("user_id")

	if mode <> "modify" and mode <> "reply" then


		if (boardcode = 88 or boardcode = 91 or boardcode = 92 or boardcode = 63 or boardcode = 86 or boardcode = 65 or boardcode = 64 or boardcode = 85 or boardcode = 66 or boardcode = 87 Or boardcode = 24 or boardcode = 25 or boardcode = 26 or boardcode = 27 or boardcode = 28 or boardcode = 29 or boardcode = 30 or boardcode = 31 or boardcode = 32 Or boardcode =33) and user_id <> "" then

			plusPoint = 500

			set oConn = Server.CreateObject("ADODB.Connection")
			'oConn.open "provider=sqloledb;server=210.124.177.231,14533;database=kda_db;uid=sa;pwd=dmlinedmpass;"
            'oConn.open Application("DSN_STR")
            oConn.open "provider=sqloledb;server=210.127.56.231,14533;database=kda_ver2;uid=kda_ver;pwd=!@#$%^!@#$%^;"
			Set point_rs = Server.CreateObject("ADODB.RecordSet")

			sql1 = "select *  from id_tbl where id='" & user_id & "'"
			point_rs.Open sql1,oConn


			if isnull(point_rs("point")) then
				point = 0
			else
				point	= Cint(point_rs("point"))	'글 작성 포인트
			End if

			point = point + plusPoint

			sql2 = "update id_tbl set point=" & point & " where id='" & user_id & "'"
			oConn.Execute(sql2)
			'response.write sql2

			point_rs.close
			set point_rs = nothing

		end if

	end if
'-----------------------------------------------------------------------------------------------





'------------------------------ 답글일때 포인트 증가 하는 부분 ---------------------------------


	if mode = "reply" then

		if (boardcode = 20 or boardcode = 88 or boardcode = 91 or boardcode = 92 or boardcode = 63 or boardcode = 86 or boardcode = 65 or boardcode = 64 or boardcode = 85 or boardcode = 66 or boardcode = 87 Or boardcode = 24 or boardcode = 25 or boardcode = 26 or boardcode = 27 or boardcode = 28 or boardcode = 29 or boardcode = 30 or boardcode = 31 or boardcode = 32 Or boardcode =33) and user_id <> "" then

			plusPoint = 100

			set oConn = Server.CreateObject("ADODB.Connection")
			'oConn.open "provider=sqloledb;server=210.124.177.231,14533;database=kda_db;uid=sa;pwd=dmlinedmpass;"
            'oConn.open Application("DSN_STR")
            oConn.open "provider=sqloledb;server=210.127.56.231,14533;database=kda_ver2;uid=kda_ver;pwd=!@#$%^!@#$%^;"
			Set point_rs = Server.CreateObject("ADODB.RecordSet")

			sql1 = "select *  from id_tbl where id='" & user_id & "'"
			point_rs.Open sql1,oConn


			if isnull(point_rs("point")) then
				point = 0
			else
				point	= Cint(point_rs("point"))	'글 작성 포인트
			End if

			point = point + plusPoint

			sql2 = "update id_tbl set point=" & point & " where id='" & user_id & "'"
			oConn.Execute(sql2)
			'response.write sql2

			point_rs.close
			set point_rs = nothing

		end if

	end if
'--------------------------------------------------------------------------------------------


	if ListUrl <> "" then
		response.Redirect(ListUrl & "?boardcode="&boardcode&"&page=" & page)
	else
		response.Redirect("index.asp?boardcode="&boardcode&"&page=" & page)
	end If








%>