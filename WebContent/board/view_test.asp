...
<%
	    ...
	    userid= Trim(request.Cookies("dietitian")("user_id"))
...
'<!--추가 E  KSH-->
		...
		if request("ListUrl") = "" then
			'ListUrl = "index.asp"		
			if boardcode = 113 then
				ListUrl = "../../etc06_01_01.asp"
			elseif boardcode = 111 Then
				ListUrl = "../../etc06_02_01.asp"
			end if
		else
		...
		if boardcode = 111 then
			WriteUrl = "etc05_05_02.asp"
		elseif boardcode = 112 then
			WriteUrl = "etc05_08_02.asp"
		elseif request("WriteUrl") = "" then
			WriteUrl = "input.asp"
		else
...
<br>
					...
					<%
						If boardcode=113 Then
							Set cmtcnt=server.CreateObject("adodb.recordset")
							cntsql = "select count(*) cnt from "& Bs_TableNameComments &" where bbs_idx = '"& idx &"' and bbc_user_id='"&userid&"' "
							cmtcnt.Open cntsql,db,0
							'Response.write cntsql
							if not  (cmtcnt.EOF or cmtcnt.BOF) then

								cnt1			= cmtcnt("cnt")		
								'Response.write "cnt=======>"&cnt1
								'Response.End
							end if
					...
					<tr>
						<td colspan=3 align="center" bgcolor="orange">
							캠페인에 적극적으로 참여한 단체급식소에 ‘추천 댓글’을 달아주세요! 
						</td>
					</tr>
					<%End If%>
					<tr align="center" bgcolor="CAE6FC">
...
<%ElseIf boardcode="113" And cnt1>=1 Then %>
<% Else %>			
