...
' ------------------- 정보공유게시판 자유게시판 통합 체크 ------------------
	...
	if boardcode = 35 or boardcode = 20 Then
		...
		sql = "select * from (select top "& rownum &" * from ("
		'sql = "select * from (select top "& totaltext - (page-1) * pagesize &" * from ("
		'sql = sql & "select top "& totaltext - (page-1) * pagesize & " bbs_idx, bbs_notice, bbs_m_number, bbs_step, bbs_file_type, bbs_comments_count, bbs_category, bbs_name, bbs_subject, bbs_view, bbs_edit_date_dt, bbs_reg_date_dt, bbs_del_flag, isnull(bbs_recomm,0) bbs_recomm from "& Bs_TableName &" where bbs_board_code = '"& boardcode &"' "
...
'	sql = sql &" ) a order by bbs_notice , bbs_m_number desc) b order by bbs_notice desc, bbs_m_number"
	sql = sql &" ) a order by bbs_m_number desc) b order by bbs_m_number "
