...
<input type="hidden" id="ar_conform_date" name="ar_conform_date" />
					...
					<table class="table t1 text small s1 ca center">
						...
						<c:choose>
							<c:when test="${dataCount>0 }">							
								<c:forEach var="list" items="${dataRow}" varStatus="status">
									<tr>
										...
										<c:choose>
										<c:when test="${list.ar_conform_year eq null || list.ar_conform_year == ''}">
										...
										<td>
											<c:if test="${list.ar_finish_yn=='2'}">
												<a href="javascript:reportView('${list.pers_name }','${list.lic_no }','${list.ar_code_part }','${list.cs_kitchen_code }','${list.yyyy}','${list.ar_conform_date}','view');" class="btn middle" >보기</a>
												<a href="javascript:reportView('${list.pers_name }','${list.lic_no }','${list.ar_code_part }','${list.cs_kitchen_code }','${list.yyyy}','${list.ar_conform_date}','print');" class="btn middle ml5">출력</a>
											</c:if>
										</td>
										</c:when>
										<c:otherwise>
										<c:choose>		
										<%--20160622 출력 확인서 중복 제거 --%>
											<%--<c:when test="${list.cs_con_education == list.ar_conform_year-1  ||   list.cs_con_education == list.ar_conform_year-2 
										|| (list.cs_con_education == '2013' && list.ar_conform_year=='2016'&& list.ar_code_part ne null)}">  --%>
										...
										<td>
											<%-- <c:choose>
												<c:when test="${null ne list.ar_conform_date && list.ar_conform_date!='' }">
													${list.ar_conform_date }
												</c:when>
												<c:otherwise>
													-
							...
							<c:otherwise>
								<tr>
									<td colspan="6">접수중인 실태신고가 없습니다.</td>
								</tr>
							</c:otherwise>
						</c:choose>
						</tbody>
					...
					<div class="box t1 bgs mt10 p10 pl15 text ssmall">
						<div class="area">
							...
							<ol class="mt10">
								<li><span class="ib w60 bold">1. 접수중</span> : 현재 실태신고 현황 확인중입니다. 제출 후 7일이 소요됩니다.
								<li><span class="ib w60 bold">2. 완   료</span> : 실태신고가 완료되었습니다. 확인서를 출력해 주십시오.
								...
								<li><span class="ib w60 bold">4. 회   송</span> : 실태신고서에 오류가 있어 접수가 거부되었습니다. 실태신고 수정에서 실태신고서를 다시 작성해 주시기 바랍니다. 
							</ol>
						</div>
					</div>
...
<div id="km_docu" style="display: none;">
	<div class="km_docu_page">
		...
		<p class="mt50 text left" id="text1">
			귀하는 『국민영양관리법 제20조의 2(실태 등의 신고)』 조항에 따라 면허를 받은지 3년마다 신고하여야 하는 영양사 실태신고와 취업상황 등을 년도에 신고 하였음을 확인합니다.
		</p>
		<p class="mt50 text left" id="text2">
			※ 다음 영양사 실태신고 연도는 년입니다.
		</p>
