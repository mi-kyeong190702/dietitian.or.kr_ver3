...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#pers_tel,#pers_hp,#pers_year,#lic_print_dt").keyup(function(event){
			$(this).val( $(this).val().replace(/[^0-9:\-]/gi,"") );
		});
		$("#pers_tel,#pers_hp,#pers_year,#lic_print_dt").focusout(function(event){
			$(this).val( $(this).val().replace(/[^0-9:\-]/gi,"") );
		});
	...
	function doCheck(code){
		...
		if($("#pers_hp").val()=="" || $("#pers_hp").val().length<11){
			alert("휴대폰 연락처를 입력해야 합니다.");
			$("#pers_hp").focus();
			return false;
		}
		...
		*/
		...
		if(code=='0'){
			if ( confirm("면허신고서 저장 후에는 마이페이지에서 면허신고수정에서 수정이 가능합니다.저장 후 반드시 제출하셔야 면허신고가 접수됩니다.\n저장하시겠습니까?")) {
				return true;	
			}else{
				return false;
			}				
	...
	function doCancel(){
		if ( confirm("취소시 비밀번호가 저장되지 않으며, 등록부터 다시 해야 합니다.취소하시겠습니까?")) {
			...
			f.submit();
					...
					<div class="box t1 bgs mt5 p10 pl15 text ssmall">
						<div class="area">
						<c:choose>
							<c:when test="${null eq dateRow.yyyy }">
								<strong class="bold cm">${loginVo.pers_name }</strong>님 반갑습니다. 면허신고는 처음이시군요. 면허신고서를 작성하신 후 제출해 주시면 신고가 접수됩니다.<br />
							</c:when>
							<c:otherwise>
								<c:set var="regDate" value="${dateRow.ar_regi_date}" />
								<strong class="bold cm">${loginVo.pers_name }</strong>님은
								<strong class="bold">${fn:substring(regDate,0,4) } </strong>년 
								<strong class="bold">${fn:substring(regDate,5,7) }</strong>월 
								<strong class="bold">${fn:substring(regDate,8,10) }</strong>일 면허신고를 하였습니다. 면허신고서를 작성하신 후 제출해 주시면 신고가 접수됩니다.
							</c:otherwise>
						</c:choose>
						</div>
					</div>
								...
								<div class="form mt10 line bt bcm">
									...
									<div class="f_wrap line bb p8">
										...
										<div class="f_field div2">
											...
											<div class="ff_wrap ml130">
												<div class="area">
													<%-- <c:set var="licDate" value="${personRow.lic_print_dt}" />
													<input type="text" class="input t1 fl w200" id="inputid3" value="${fn:substring(licDate,0,4) }년 ${fn:substring(licDate,4,6) }월 ${fn:substring(licDate,6,8) }일"  /> --%>
													...
													<%-- <input type="hidden" id="lic_print_dt" value="${personRow.lic_print_dt}"/> --%>
												</div>
											</div>
										</div>
									</div>
												...
												<ul>
													<li>
														<div class="area">
															...
															<input type="text" class="input t1 fl w150" id="pers_tel" value="${personRow.pers_tel }" maxlength="11" />
															<p class="fl text cp s1 ml10">예) 021234567</p>
															<%-- <select class="select t2 fl w100" id="tel1">
																<option value="02" <c:if test="${tel[0] == '02'}"><c:out value="selected"/></c:if> >02</option>
																<option value="031" <c:if test="${tel[0] == '031'}"><c:out value="selected"/></c:if> >031</option>
																<option value="032" <c:if test="${tel[0] == '032'}"><c:out value="selected"/></c:if> >032</option>
																<option value="033" <c:if test="${tel[0] == '033'}"><c:out value="selected"/></c:if> >033</option>
																<option value="041" <c:if test="${tel[0] == '041'}"><c:out value="selected"/></c:if> >041</option>
																<option value="042" <c:if test="${tel[0] == '042'}"><c:out value="selected"/></c:if> >042</option>
																<option value="043" <c:if test="${tel[0] == '043'}"><c:out value="selected"/></c:if> >043</option>
																<option value="051" <c:if test="${tel[0] == '051'}"><c:out value="selected"/></c:if> >051</option>
																<option value="052" <c:if test="${tel[0] == '052'}"><c:out value="selected"/></c:if> >052</option>
																<option value="053" <c:if test="${tel[0] == '053'}"><c:out value="selected"/></c:if> >053</option>
																<option value="054" <c:if test="${tel[0] == '054'}"><c:out value="selected"/></c:if> >054</option>
																<option value="055" <c:if test="${tel[0] == '055'}"><c:out value="selected"/></c:if> >055</option>
																<option value="061" <c:if test="${tel[0] == '061'}"><c:out value="selected"/></c:if> >061</option>
																<option value="062" <c:if test="${tel[0] == '062'}"><c:out value="selected"/></c:if> >062</option>
																<option value="063" <c:if test="${tel[0] == '063'}"><c:out value="selected"/></c:if> >063</option>
																<option value="064" <c:if test="${tel[0] == '064'}"><c:out value="selected"/></c:if> >064</option>
																<option value="070" <c:if test="${tel[0] == '070'}"><c:out value="selected"/></c:if> >070</option>
															</select>
															<label for="tel2" class="fl text ml10 mr10">-</label>
															<input type="text" class="input t1 fl w100" id="tel2" />
															<label for="tel3" class="fl text ml10 mr10">-</label>
													...
													<li class="mt5">
														<div class="area">
															...
															<input type="text" class="input t1 fl w150" id="pers_hp" value="${personRow.pers_hp }" maxlength="11" />
															...
															<input type="text" class="input t1 fl w100" id="hp2" />
															<label for="hp3" class="fl text ml10 mr10">-</label>
															<input type="text" class="input t1 fl w100" id="hp3" /> --%>
														</div>
													</li>
												</ul>
											</div>
...
<!-- 											<div class="ff_wrap ml130"> -->
<!-- 												<div class="area"> -->
<%-- 													<input type="text" class="input t1 fl w100" id="pers_year" value="${personRow.pers_year }" maxlength="4" /> --%>
...
<!-- 												<div class="area"> -->
<%-- 													<input type="radio" class="mr5" name="radio1" id="radio1_1" value="1" <c:if test="${personRow.code_sex == '1'}"><c:out value="checked"/></c:if> /> --%>
<!-- 													<label for="radio1_1" class="mr20">남자</label> -->
<%-- 													<input type="radio" class="mr5" name="radio1" id="radio1_2" value="0" <c:if test="${personRow.code_sex == '0'}"><c:out value="checked"/></c:if> /> --%>
...
<!-- 									</div> -->
									...
									<div class="f_wrap line bb p8">
										<div class="f_field div1">
											<label for="bbs_file1" class="ff_title i_b"><strong>면허증사본 첨부</strong></label>
											...
											<div class="ff_wrap ml130">
												...
												<c:if test="${null ne reportRow.ar_add_file_name}">
								...
								<div class="form mt10 line bt bcm">
									<div class="f_wrap line bb p8">
										<div class="f_field div1">
											...
											<div class="ff_wrap ml130 text">
												<div class="area">
													<input type="radio" class="mr5" name="radio2" id="radio2_1" onclick="doActive(this.value);" value="1" <c:if test="${reportRow.ar_code_part == '1'}"><c:out value="checked"/></c:if> />
													<label for="radio2_1" class="mr20">집단급식소 근무</label>
													<input type="radio" class="mr5" name="radio2" id="radio2_2" onclick="doActive(this.value);" value="2" <c:if test="${reportRow.ar_code_part == '2'}"><c:out value="checked"/></c:if> />
													<label for="radio2_2" class="mr20">비집단급식소 근무</label>
													<input type="radio" class="mr5" name="radio2" id="radio2_3" onclick="doActive(this.value);" value="3" <c:if test="${reportRow.ar_code_part == '3'}"><c:out value="checked"/></c:if> />
									...
									<div class="f_wrap line bb p8 activeNo" id="active1" style="display:none;">
										<div class="f_field div1">
											...
											<div class="ff_wrap ml130 text">
												<div class="area">
													<ul>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_1" value="01"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '01'}"><c:out value="checked"/></c:if> /><label for="radio3_1">산업체</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_2" value="02"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '02'}"><c:out value="checked"/></c:if> /><label for="radio3_2">병원</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_3" value="03"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '03'}"><c:out value="checked"/></c:if> /><label for="radio3_3">학교</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_4" value="04"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '04'}"><c:out value="checked"/></c:if> /><label for="radio3_4">사회복지시설</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_5" value="05"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '05'}"><c:out value="checked"/></c:if> /><label for="radio3_5">어린이집</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_6" value="06"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '06'}"><c:out value="checked"/></c:if> /><label for="radio3_6">유치원</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_7" value="07"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '07'}"><c:out value="checked"/></c:if> /><label for="radio3_7">교정</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_8" value="08"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '08'}"><c:out value="checked"/></c:if> /><label for="radio3_8">군대</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_9" value="09"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '09'}"><c:out value="checked"/></c:if> /><label for="radio3_9">경찰서</label></li>
														<li class="fl w170"><input type="radio" class="mr5" name="active1" id="radio3_11" value="11"  onclick="doSubActive(this.value);" <c:if test="${reportRow.cs_kitchen_code == '11'}"><c:out value="checked"/></c:if> /><label for="radio3_11">소방서</label></li>
													</ul>
												...
												<ul id="addrUl1">
													<li>
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="cs_kitchen_name1" value="${reportRow.cs_kitchen_name1}" />
														</div>
														<div class="area mt5">
															<label for="code_post1" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post1" value="${reportRow.code_post1}" readonly/>
																	<a href="javascript:openJusoPopup('2');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="cs_kitchen_add11" class="ti">기본주소</label>
																	<input type="text" class="input t1 fl w150" id="cs_kitchen_add11" value="${reportRow.cs_kitchen_add11}" readonly />
																	<label for="cs_kitchen_add12" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="cs_kitchen_add12" value="${reportRow.cs_kitchen_add12}" />
																</div>
															</div>
														</div>
													</li>
													<li class="mt5 pt5 line bt">
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="cs_kitchen_name2" value="${reportRow.cs_kitchen_name2}" />
														</div>
														<div class="area mt5">
															<label for="code_post2" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post2" value="${reportRow.code_post2}" readonly/>
																	<a href="javascript:openJusoPopup('3');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="cs_kitchen_add21" class="ti">기본주소</label>
																	<input type="text" class="input t1 fl w150" id="cs_kitchen_add21" value="${reportRow.cs_kitchen_add21}" readonly/>
																	<label for="cs_kitchen_add22" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="cs_kitchen_add22" value="${reportRow.cs_kitchen_add22}" />
																</div>
															</div>
														</div>
													</li>
													<li class="mt5 pt5 line bt">
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="cs_kitchen_name3" value="${reportRow.cs_kitchen_name3}" />
														</div>
														<div class="area mt5">
															<label for="code_post3" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post3" value="${reportRow.code_post3}" readonly/>
																	<a href="javascript:openJusoPopup('4');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="cs_kitchen_add31" class="ti">기본주소</label>
																	<input type="text" class="input t1 fl w150" id="cs_kitchen_add31" value="${reportRow.cs_kitchen_add31}" readonly/>
																	<label for="cs_kitchen_add32" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="cs_kitchen_add32" value="${reportRow.cs_kitchen_add32}" />
																</div>
															</div>
														</div>
													</li>
													<li class="mt5 pt5 line bt">
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="cs_kitchen_name4" value="${reportRow.cs_kitchen_name4}" />
														</div>
														<div class="area mt5">
															<label for="code_post4" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post4" value="${reportRow.code_post4}" readonly/>
																	<a href="javascript:openJusoPopup('5');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="cs_kitchen_add41" class="ti">기본주소</label>
																	<input type="text" class="input t1 fl w150" id="cs_kitchen_add41" value="${reportRow.cs_kitchen_add41}" readonly/>
																	<label for="cs_kitchen_add42" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="cs_kitchen_add42" value="${reportRow.cs_kitchen_add42}" />
																</div>
															</div>
														</div>
													</li>
													<li class="mt5 pt5 line bt">
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="cs_kitchen_name5" value="${reportRow.cs_kitchen_name5}" />
														</div>
														<div class="area mt5">
															<label for="code_post5" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post5" value="${reportRow.code_post5}" readonly/>
																	<a href="javascript:openJusoPopup('6');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="cs_kitchen_add51" class="ti">기본주소</label>
																	<input type="text" class="input t1 fl w150" id="cs_kitchen_add51" value="${reportRow.cs_kitchen_add51}" readonly/>
																	<label for="cs_kitchen_add52" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="cs_kitchen_add52" value="${reportRow.cs_kitchen_add52}" />
																</div>
															</div>
														</div>
													</li>
									...
									<div class="f_wrap line bb p8 activeNo" id="active2" style="display:none;">
										<div class="f_field div1">
											...
											<div class="ff_wrap ml130 text">
												<div class="area">
													<ul>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_1" value="01" <c:if test="${reportRow.ncs_kitchen_code == '01'}"><c:out value="checked"/></c:if> /><label for="radio4_1">건강기능식품 판매업소</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_2" value="02" <c:if test="${reportRow.ncs_kitchen_code == '02'}"><c:out value="checked"/></c:if> /><label for="radio4_2">교육 및 연구기관</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_3" value="03" <c:if test="${reportRow.ncs_kitchen_code == '03'}"><c:out value="checked"/></c:if> /><label for="radio4_3">어린이급식관리지원센터</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_4" value="04" <c:if test="${reportRow.ncs_kitchen_code == '04'}"><c:out value="checked"/></c:if> /><label for="radio4_4">위탁급식전문업체(행정업무)</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_5" value="05" <c:if test="${reportRow.ncs_kitchen_code == '05'}"><c:out value="checked"/></c:if> /><label for="radio4_5">보건소</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_6" value="06" <c:if test="${reportRow.ncs_kitchen_code == '06'}"><c:out value="checked"/></c:if> /><label for="radio4_6">행정직 공무원</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_7" value="07" <c:if test="${reportRow.ncs_kitchen_code == '07'}"><c:out value="checked"/></c:if> /><label for="radio4_7">육아종합지원센터</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active2" id="radio4_8" value="08" <c:if test="${reportRow.ncs_kitchen_code == '08'}"><c:out value="checked"/></c:if> /><label for="radio4_8">식품제조 가공업소</label></li>
														<li class="fl w240"><input type="radio" class="fl mr5" name="active2" id="radio4_9" value="09" <c:if test="${reportRow.ncs_kitchen_code == '09'}"><c:out value="checked"/></c:if> /><label for="radio4_9" class="fl">기타</label><input type="text" class="input t1 fl w100 ml10" id="ncs_kitchen_ret_code" value="${reportRow.ncs_kitchen_ret_code}" maxlength="40"/></li>
													</ul>
												</div>
											</div>
											<div class="ff_wrap ml130">
												<ul id="addrUl2">
													<li>
														<div class="area">
															...
															<input type="text" class="input t1 fl w200" id="ncs_kitchen_name1" value="${reportRow.ncs_kitchen_name1}" />
														</div>
														<div class="area mt5">
															<label for="code_post6" class="fl w100 text bold cm">주소</label>
															<div class="ml100">
																<div class="area">
																	<input type="text" class="input t1 fl w200" id="code_post6" value="${reportRow.code_post6}" readonly/>
																	<a href="javascript:openJusoPopup('7');" class="btn form t1 fl ml5">주소검색</a>
																</div>
																<div class="area mt5">
																	<label for="ncs_kitchen_add61" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w150" id="ncs_kitchen_add61" value="${reportRow.ncs_kitchen_add61}" readonly/>
																	<label for="ncs_kitchen_add62" class="ti">상세주소</label>
																	<input type="text" class="input t1 fl w200 ml5" id="ncs_kitchen_add62" value="${reportRow.ncs_kitchen_add62}" />
																</div>
															</div>
														</div>
													</li>
									...
									<div class="f_wrap line bb p8 activeNo" id="active3" style="display:none;">
										<div class="f_field div1">
											...
											<div class="ff_wrap ml130 text">
												<div class="area">
													<ul id="addrUl3">
														<li class="fl w240"><input type="radio" class="mr5" name="active3" id="radio5_1" value="1" <c:if test="${reportRow.cs_kitchen_act_code == '1'}"><c:out value="checked"/></c:if> /><label for="radio5_1">임신,출산,육아</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active3" id="radio5_2" value="2" <c:if test="${reportRow.cs_kitchen_act_code == '2'}"><c:out value="checked"/></c:if> /><label for="radio5_2">일시적 폐업, 실직</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active3" id="radio5_3" value="3" <c:if test="${reportRow.cs_kitchen_act_code == '3'}"><c:out value="checked"/></c:if> /><label for="radio5_3">질병, 사고 등</label></li>
														<li class="fl w240"><input type="radio" class="mr5" name="active3" id="radio5_4" value="4" <c:if test="${reportRow.cs_kitchen_act_code == '4'}"><c:out value="checked"/></c:if> /><label for="radio5_4">해외체류</label></li>
														<li class="fl w240"><input type="radio" class="fl mr5" name="active3" id="radio5_5" value="5" <c:if test="${reportRow.cs_kitchen_act_code == '5'}"><c:out value="checked"/></c:if> /><label for="radio5_5" class="fl">기타</label><input type="text" class="input t1 fl w100 ml10" id="cs_kitchen_ret_code" value="${reportRow.cs_kitchen_ret_code}" maxlength="40"/></li>
													</ul>
												</div>
											</div>
										</div>
								...
								<h6 class="title s1 i_b_t3 mt40"><mark class="bold">보수교육 및 신고관련</mark> <span class="ssmall">( <span class="i_b_t5"> 표시는 필수항목입니다.</span> )</span></h6>
								<div class="form mt10 line bt bcm">
									<div class="f_wrap line bb p8">
										<div class="f_field div1">
											...
											<div class="ff_wrap ml130">
												<div class="area">
													<ul>
													<c:set var="length" value="${fn:length(recentRow)}"/>
													<c:if test="${length<1 }"><li class="fl w150"><strong class="bold cm">신고 이력이 없습니다.</strong></li></c:if>
													<c:forEach var="list" items="${recentRow}">
														<li class="fl w150"><input type="text" class="input t1 w100" id="inputid36" value="<c:out value="${list.yyyy }"/>" readonly /><label for="inputid36" class="ml5">년</label></li>
													</c:forEach>
													</ul>
												</div>
											</div>
														...
														<c:choose>
															...
															<c:otherwise>
																...
																<c:forEach var="list2" items="${recentRow}" varStatus="status">
																	<strong class="bold cm">${list2.report_year }</strong> 년 ${list2.ar_finish_point }
																...
																<c:forEach var="list3" items="${recentRow}" varStatus="status">
																	<strong class="bold cm">${list3.report_year }</strong> 년 ${list3.ar_state_kr} ${list3.ar_marks_point }
																	<c:if test="${comma !=0 }">,</c:if>
																	<c:if test="${comma ==0 }"><c:set var="comma" value="1"/></c:if>
																</c:forEach>
																) 시간 이수
