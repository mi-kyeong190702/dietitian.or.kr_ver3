...
<script type="text/javascript">
	...
	function doCheck(){
		if($(':checkbox[name="checkbox1"]:checked').length < 1 ) {
			alert("이용약관에 동의 하셔야 합니다");
			return false;
		}
		if($(':checkbox[name="checkbox2"]:checked').length < 1 ) {
	...
	function mobileConfirm(){
		...
		if(doCheck()){
			...
			var popupWindow = window.open( "", "MOBILEPOP", "left=200, top=100, status=0, width=650, height=450" );
			
			var MOBILE = document.MOBILE;

			MOBILE.target = "MOBILEPOP";
			MOBILE.action = "/pages/work/research/report/kcpcert_start.jsp";
			MOBILE.submit();
	...
	function doIpinCheckComplete()
	{
		...
		var voF = document.forms["actulpersonVo"];
               		
   		if(IPIN.IPIN_PERS_NAME.value == voF.pers_name.value) {
   			...
   			alert("입력하신 이름과 I-PIN정보가 일치하지 않습니다.\r\n다시 확인하시고, 정확히 입력해 주시기 바랍니다.");
   			return;
   		}
	...
	/* function mobileConfirm(){
		if(doCheck()){
			...
			var popupWindow = window.open( "/pages/work/research/report/kcpcert_start.jsp", "MOBILEPOP", "left=200, top=100, status=0, width=650, height=450" );
...
<form name="actulpersonVo" method="post" >
<input type="hidden" name="pers_name" value="${actulpersonVo.pers_name}" />
</form>
					...
					<div class="box t2 bgs p20 text">
						...
						&nbsp;④ 다른 이용자의 ID, 비밀번호, 생년월일를 도용하는 행위<br>
						...
						(1) 본 약관은 2008년 10월 8일부터 적용됩니다.
								...
								<div class="line bt dot mt10 p8 pb0 text ssmall left">
									...
									<label for="checkbox1_1" class="mr20">이용약관에 동의합니다.</label>
								</div>
							</div>
						</div>
					</div>
					...
					&nbsp;&nbsp;
					관련 법령 및 협회 정관에 따른 회원관리 업무, 교육 실시 업무, 자격 관련 업무,<br>
					&nbsp;&nbsp;
					...
					- 보유 기간 : 영구<br>
								...
								<div class="line bt dot mt10 p8 pb0 text ssmall left">
									<input type="checkbox" class="mr5" name="checkbox2" id="checkbox2_1" />
									<label for="checkbox2_1" class="mr20">개인정보 수집 및 이용안내에 동의합니다.</label>
								</div>
							</div>
						</div>
					</div>
					<div class="box t2 bgs mt20 p20 text">
						<div class="area">
							...
							<div class="box t1 r5 pr mt10 p40 ca">
								...
								<span class="ml110 s1">I-PIN 인증으로 진행하려면 <mark class="cb">'아이핀인증'</mark>을 클릭해 주세요.</span>
								
								<!-- 면허신고 년도 변경시 수정 : 면허신고 CLOSE - 링크삭제 Y - start  -->
<!-- 								<span class="pa r40 t30"><a href="#" class="btn t1 big">아이핀 인증</a></span> -->
								<!-- 면허신고 년도 변경시 수정 : 면허신고 CLOSE - 링크삭제 Y - end  -->
								
								<!-- 면허신고 년도 변경시 수정 : 면허신고 OPEN - 링크삭제 N - start  -->
								<span class="pa r40 t30"><a href="javascript:ipinConfirm();" class="btn t1 big">아이핀 인증</a></span>
								<!-- 면허신고 년도 변경시 수정 : 면허신고 OPEN - 링크삭제 N - end  -->
								
							</div>
							<div class="box t1 r5 pr mt20 p40 ca">
								<strong class="pa l40 t40 bold cm">휴대폰 인증</strong>
								<span class="ml110 s1">본인 휴대폰으로 발송된 인증번호를 연락하여 인증 진행합니다.</span>
								
								<!-- 면허신고 년도 변경시 수정 : 면허신고 CLOSE - 링크삭제 Y - start  -->
<!-- 								<span class="pa r40 t30"><a href="#" class="btn t1 big">휴대폰 인증</a></span> -->
								<!-- 면허신고 년도 변경시 수정 : 면허신고 CLOSE - 링크삭제 Y - end  -->
								
								<!-- 면허신고 년도 변경시 수정 : 면허신고 OPEN - 링크삭제 N - start  -->
								<span class="pa r40 t30"><a href="javascript:mobileConfirm();" class="btn t1 big">휴대폰 인증</a></span>
								<!-- 면허신고 년도 변경시 수정 : 면허신고 OPEN - 링크삭제 N - end  -->
