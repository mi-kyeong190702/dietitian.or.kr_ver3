...
function doc_print(str1, str2)
{
    ...
    path   = "/print/member_rcp_print.do?dues_gubun="+str1+"&dues_h_seq="+str2;
    ...
    window.open(path, "member_rcp_print", shape);
}


function doc_print_member(str1, str2)
{
    shape  = "width=900,height=600,";
    shape += "toolbar=no,location=no,directories=no,status=no,";
    shape += "menubar=no,scrollbars=no,resizable=yes";
    path   = "/print/member_print.do?dues_gubun="+str1+"&dues_h_seq="+str2;
    ...
    window.open(path, "member_print", shape);
}
</script>
	...
	<div class="sp_head">
		...
		<div class="sp_navi">
			<ul>
				...
				<li><a href="/kmp_payment.do?doc_code=0105" class="cm">회비 납입내역</a></li>
			</ul>
		</div>
	</div>
	<hr />
	<div class="sp_body">
		...
		<table class="table t1 text s1 ca center mt20">
			...
			<tr>
				...
				<!-- <th scope="col" class="bold cm">회원증</th> -->
				...
				<c:when test="${requestScope.myList.size() > 0 }">
					<c:forEach var="i" begin="0" end="${requestScope.myList.size()-1 }">
						<tr>
							...
							<!-- 
							<td>
								<c:if test="${requestScope.myList[i].conform_yn == 3 && requestScope.myList[i].code_member != '71' && requestScope.myList[i].code_member != '72' }">
								    <a href="javascript:doc_print_member('${requestScope.myList[i].dues_gubun}','${requestScope.myList[i].dues_h_seq}')" class="btn t2">출력</a>
								</c:if>
							</td>
							 -->
							<td>
                                <c:if test="${requestScope.myList[i].conform_yn == 3}">
                                     <a href="javascript:doc_print('${requestScope.myList[i].dues_gubun}','${requestScope.myList[i].dues_h_seq}')" class="btn">출력</a>
                                </c:if>
                            </td>
						</tr>
					</c:forEach>
	    ...
	    <div class="paging small mt30">
		        <a href="javascript:moveFirst();" class="p_btn start"><span class="icon"></span><span class="ti">처음</span></a>
                <a href="javascript:movePrev();" class="p_btn prev"><span class="icon"></span><span class="ti">이전</span></a>         
		        ...
		        <a href="javascript:moveNext();" class="p_btn next"><span class="icon"></span><span class="ti">다음</span></a>
                <a href="javascript:moveLast();" class="p_btn end"><span class="icon"></span><span class="ti">끝</span></a>
		 </div>
		 ...
		 <div class="print-section">
		  <div class="print-info">
		    <span class="title">회원증 출력</span><br />
		    <span class="note">※ 출력 아이콘이 보이지 않을 경우, 문의처로 연락 주시기 바랍니다.</span><br />
		    <span class="contact">
		      <strong>문의처</strong>: 조직·홍보국 조직관리팀 02)823-5680 <span class="sub">(내선번호 311~313)</span>
		    </span>
		  </div>
		
		  <div class="print-button"> 
		      <c:if test="${requestScope.memberYn == 'Y'}">
		          <button onclick="doc_print_member('${requestScope.dues_gubun}','${requestScope.dues_h_seq}')">출력</button>
		      </c:if>
		  </div>
		</div> --%>		 
	</div>
...
function movePrev(page)
{
	...
	const prevPage = ${startPage - 10 > 0 ? startPage - 10 : 1};
...
{ 
	const nextPage = ${currentPage + 10 <= totalPage ? currentPage + 10 : totalPage};
