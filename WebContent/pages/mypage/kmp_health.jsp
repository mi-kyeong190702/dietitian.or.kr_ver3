...
<div class="s_page">
	<div class="sp_head">
		<div class="sp_title">
			<h3 class="title bold t1 s1 i_b_t1"><mark class="cm">산업보건유지</mark> 교육</h3>
		</div>
		<div class="sp_navi">
			<ul>
				...
				<li><a href="/mypage/kmp_health" class="cm">산업보건유지 교육</a></li>
			</ul>
		</div>
	</div>
	<hr />
		...
		<c:choose>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<form name="edu" type="post">
					<tr>
						<td> 본회 : ${requestScope.col1} 점 / 지회 : ${requestScope.col2} 점  /  총점 ${requestScope.total} 점</td>
						<td align="right">
				...
				<c:when test="${requestScope.myList2.size() >0 }">
					<c:forEach var="i" begin="0" end="${requestScope.myList2.size()-1 }">
						<tr <c:if test="${requestScope.myList2[i].rt == '11' || requestScope.myList2[i].rt == '10'}">style="background-color:#ffd3d0"</c:if>
							...
							<td><!-- 점수반영 -->
								<c:choose>
									<c:when test="${requestScope.myList2[i].rt == '10' &&  requestScope.myList2[i].point_manage == '0' && requestScope.view_num == '0'}"><a href="javascript:point_apply('3'
	,'${requestScope.myList2[i].code_kind}${requestScope.myList2[i].code_certifi}${requestScope.myList2[i].code_seq}${requestScope.myList2[i].code_bran}${requestScope.myList2[i].bran_seq}${requestScope.myList2[i].receipt_no}')" class="btn">점수반영</a></c:when>
				...
				<c:otherwise>
					<tr>
						<td colspan="7">산업보건유지 교육 목록이 없습니다.</td>
					</tr>
				</c:otherwise>
			</c:choose>
			</tbody>
