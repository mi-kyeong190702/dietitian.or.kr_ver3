...
<div class="box t3 text small">
	<ul class="area">
		...
		<li class="fl w33p bgw">
			<div class="p10 pl15">
				<strong class="ib w70 bold cm">작성일</strong>
				<span>${f:substring(requestScope.boardRow.bbs_reg_date_dt, 0, 10)}</span>
			</div>
		</li>
		<li class="fl w33p bgw">		
			<div class="p10 pl15">
				<strong class="ib w70 bold cm ">직성자</strong>
		...
		<c:if test="${requestScope.boardRow.bbs_board_code!=116}">
		<li class="fl w33p bgw">
			<div class="p10 pl15">
				<strong class="ib w70 bold cm">이메일</strong>
				<c:if test="${requestScope.boardRow.bbs_user_id == g_userid}"><span>${requestScope.boardRow.bbs_email}</span></c:if>
				<c:if test="${requestScope.boardRow.bbs_user_id != g_userid}"><span>***********</span></c:if>
			</div>
		</li>
		</c:if>
	</ul>
...
<div id="print_contents" class="line bb p30 text">
	<h4 class="ti"></h4>	
		<iframe name="bbs_contents" id="bbs_contents" width="682" scrolling="auto" style="overflow-x: scroll !important; overflow-y: hidden !important;" src="/paper_board_view_contents.do?bbs_idx=${requestScope.boardRow.bbs_idx}" onload="javascript:setIFrameHeight(this)"></iframe>
	<br/>
</div>
