...
<%@ page import="java.io.PrintWriter, 
				...
				kda.work.publication.payment.data.CARD_PAYMENT,
				kda.common.data.TBL_ORDER_LIST,
				kda.common.PaymentService" %>
...
<html xmlns="http://www.w3.org/1999/xhtml" >
    <head>
        ...
        function goResult() {
            var openwin = window.open( '/page/payment/proc_win.html', 'proc_win', '' )
...
<%
	...
	g_conf_site_cd		= f_get_parm( request.getParameter( "site_cd"         ) ); 
	g_conf_site_key		= f_get_parm( request.getParameter( "site_key"         ) );  

	//영협자체 파라미터
	String id         	= f_get_parm( request.getParameter( "id"         ) ); 
	String code_bran  	= f_get_parm( request.getParameter( "code_bran"  ) );
	String o_gubun  	= f_get_parm( request.getParameter( "o_gubun"  ) );	
	String pers_birth   = f_get_parm( request.getParameter( "pers_birth"  ) );
	String goods_code_num = f_get_parm( request.getParameter( "goods_code_num"  ) );
    /* ============================================================================== */
    /* =   02. 지불 요청 정보 설정                                                  = */
    /* = -------------------------------------------------------------------------- = */
    ...
    */
    ...
    /* =   05. 실행 END                                                             = */
    /* ============================================================================== */
    System.out.println( "=============승인결과 값 : [ req_tx : " + req_tx +" | res_cd : " + res_cd + " ]=========================" );
    ...
    /* = -------------------------------------------------------------------------- = */
        if ( res_cd.equals( "0000" ) )
        {
            ...
            if ( use_pay_method.equals( "100000000000" ) )
            {
            	...
            	TBL_ORDER_LIST order = new TBL_ORDER_LIST();
            	order.setCode_bran( code_bran );
            	order.setO_gubun( o_gubun );
            	order.setReq_tx( req_tx );
            	order.setRes_cd( res_cd );
            	order.setRes_msg( res_msg );
            	order.setOrdr_idxx(ordr_idxx);
            	order.setTno(tno);
            	order.setGood_mny( Integer.parseInt(good_mny) );
            	order.setGood_name(good_name);
            	order.setBuyr_name(buyr_name);
            	order.setBuyr_tel1(buyr_tel1);
            	order.setBuyr_tel2(buyr_tel2);
            	order.setBuyr_mail(buyr_mail);
            	order.setCard_cd(card_cd);
            	order.setCard_name(card_name);
            	order.setApp_time(app_time);
            	order.setApp_no(app_no);
            	order.setQuota(quota);
            	order.setPers_birth(pers_birth);
            	order.setId(id);
            	order.setGoods_code_num(goods_code_num);
           	    
            	// ==================== DB 작업 처리 =======================
            	try { 
					WebApplicationContext ctx = WebApplicationContextUtils.getWebApplicationContext(getServletContext());
					PaymentService paymentService = (PaymentService)ctx.getBean("paymentService");
	        		boolean result = paymentService.setPaymentComplete(order);
	            	if( result ) {
	            	    bSucc = "true";
	            	}
            	} catch(Exception e) { 
