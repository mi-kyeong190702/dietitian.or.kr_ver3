...
<script type="text/javascript">
	...
	function jsf__pay(form) {
		...
		if (MakePayMessage(form) == true) {
			openwin = window.open("proc_win.html", "proc_win",
					"width=449, height=209, top=300, left=300");
			RetVal = true;
		}
...
<body onload="init_orderid();">
	...
	<div id="sample_wrap">
		...
		<form name="order_info" method="post" action="pp_cli_hub.jsp">
			...
			%>
			<h1>
				[결제요청] <span> 이 페이지는 결제를 요청하는 샘플(예시) 페이지입니다.</span>
			</h1>
			<!-- 상단 문구 -->
			<div class="sample">
				<p>
					이 페이지는 결제를 요청하는 페이지입니다.<br /> 고객이 결제요청 페이지에 접근하게 되면 웹 브라우저에서 결제를
					위한 Payplus Plug-in이<br /> 고객의 PC에 설치되어 있는지 확인합니다.<br />
					<br /> 고객의 PC에 Payplus Plug-in이 설치되지 않은 경우 <span>브라우저 상단의
						노란색 알림표시줄</span>이나 하단의 <span>[수동설치]</span>를 통해 <span>Payplus
						Plug-in 설치</span>가 가능합니다.<br />
					<br /> 결제요청 버튼을 클릭하게 될 경우 Payplus Plug-in이 실행되며 Payplus Plug-in을
					통해 결제요청 정보를 암호화하여 결제요청 페이지로 전송합니다.<br />
					<br /> 소수 수정 시 <span>※ 필수, ※ 옵션</span>표시가 포함된 문장은 가맹점의 상황에 맞게 적절히
					수정 적용하시기 바랍니다.
				</p>
...
<!-- 								<option value="111000000000">신용카드/계좌이체/가상계좌</option> -->
					...
					<tr>
						<th>주문 번호</th>
						<td><input type="text" name="ordr_idxx" class="w200" value="<%=ParamUtils.getParameter(request, "ordr_idxx")%>" maxlength="40" /></td>
					</tr>
					<!-- 상품명(good_name) -->
					<tr>
						<th>상품명</th>
						<td><input type="text" name="good_name" class="w100" value="<%=ParamUtils.getParameter(request, "good_name")%>" /></td>
					</tr>
					<!-- 결제금액(good_mny) - ※ 필수 : 값 설정시 ,(콤마)를 제외한 숫자만 입력하여 주십시오. -->
					<tr>
						<th>결제 금액</th>
						<td><input type="text" name="good_mny" class="w100"	value="<%=ParamUtils.getParameter(request, "good_mny")%>" maxlength="9" />원(숫자만 입력)</td>
					</tr>
					<!-- 주문자명(buyr_name) -->
					<tr>
						<th>주문자명</th>
						<td><input type="text" name="buyr_name" class="w100" value="<%=ParamUtils.getParameter(request, "buyr_name")%>" /></td>
					</tr>
					<!-- 주문자 E-mail(buyr_mail) -->
					<tr>
						<th>E-mail</th>
						<td><input type="text" name="buyr_mail" class="w200" value="<%=ParamUtils.getParameter(request, "buyr_mail")%>" maxlength="30" /></td>
					</tr>
					<!-- 주문자 연락처1(buyr_tel1) -->
					<tr>
						<th>전화번호</th>
						<td><input type="text" name="buyr_tel1" class="w100" value="<%=ParamUtils.getParameter(request, "buyr_tel1")%>" maxlength="30" /></td>
					</tr>
					<!-- 휴대폰번호(buyr_tel2) -->
					<tr>
						<th>휴대폰번호</th>
						<td><input type="text" name="buyr_tel2" class="w100" value="<%=ParamUtils.getParameter(request, "buyr_tel2")%>" maxlength="30" /></td>
					</tr>
				</table>

				<!-- 결제 요청/처음으로 이미지 -->
				<div class="btnset" id="display_pay_button" style="display: block">
					<input name="" type="submit" class="submit" value="결제요청"
						onclick="return jsf__pay(this.form);" /> <a href="../index.html"
						class="home">처음으로</a>
				</div>
				<!-- Payplus Plug-in 설치 안내 -->
			...
			<div class="footer">Copyright (c) KCP INC. All Rights reserved.
			</div>
			<%
				/* = -------------------------------------------------------------------------- = */
				/* =   1. 주문 정보 입력 END                                                    = */
...
<input type='hidden' name='use_pay_method' value='' />
			...
			<input type="hidden" name="cash_tsdtime" value="" /> <input
				type="hidden" name="cash_yn" value="" /> <input type="hidden"
				name="cash_authno" value="" /> <input type="hidden"
