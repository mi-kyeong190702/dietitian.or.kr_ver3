...
<head>
    ...
    <meta property="og:image"              content="https://www.dietitian.or.kr/images/banner/202502191147011651.png" /> 
 ...
 // [ popup - 1 ]
    ...
    var tmpPopEndDt_1   = "2025/07/01"; // 팝업 종료할날짜(종료일 + 1 로 설정해야 함)
    ...
    if( getCookie( "notice_1" ) != "done" ){
        if( vNowDt >= vPopStartDt_1 && vNowDt <= vPopEndDt_1 ) {
            noticeWindow_1 = window.open('/assets/ver2/popup/popup_20250509.html','notice_1','toolbar=no,location=no,directories=no,status=no,menubar=no, scrollbars=yes, resizable=no,width=500px,height=530px,top=150px,left=450px');
            //winddow.open의 ()의 것은 한줄에 계속 붙여써야 오류가 안남, 줄바뀌면 오류남
    ...
    var tmpPopStartDt_2 = "2025/04/30"; // 팝업 시작할날짜(자릿수지키세요)
    var tmpPopEndDt_2   = "2025/07/26"; // 팝업 종료할날짜(종료일 + 1 로 설정해야 함)
    ...
    if( getCookie( "notice_2" ) != "done" ){
        ...
        if( vNowDt >= vPopStartDt_2 && vNowDt <= vPopEndDt_2 ) {
            noticeWindow_2 = window.open('/assets/ver2/popup/popup_20250430.html','notice_2','toolbar=no,location=no,directories=no,status=no,menubar=no, scrollbars=yes, resizable=no,width=600px,height=880px,top=100px,left=1000px');
            //winddow.open의 ()의 것은 한줄에 계속 붙여써야 오류가 안남, 줄바뀌면 오류남
