...
<html>
    <head>
        ...
        <link href="/css/research_sample.css" rel="stylesheet" type="text/css">
        <script type="text/javascript">
            ...
            function auth_data( frm )
            {
               ...
               /* 리턴 값 모두 찍어보기 (테스트 시에만 사용) */
                ...
                {
                	//if(frm.elements[i].name == "user_name" || frm.elements[i].name == "birth"){
	                    form_value += "["+frm.elements[i].name + "] = [" + frm.elements[i].value + "]\n";
	                	//form_value += "["+frm.elements[i].value + "] = []\n";
                		//form_value += frm.elements[i].value;
                	//}
                }
               	...
               	if(frm.res_cd.value=="0000"){
               		/*
               		var ff = opener.document.forms["MOBILE"];
               		ff.persName.value = frm.user_name.value;
               		ff.persBirth.value = "19871118";//frm.birth.value;
               		ff.action = "/member/km_join_step2.do";
	    			ff.method = "POST";
	    			ff.target = opener.window.MOBILEPOP; // !!주의 고정값 ( 리턴받을때 사용되는 타겟명입니다.)
	    			ff.submit();
	    			//alert("감사합니다. 휴대폰인증이 완료 되었습니다.\n회원가입 창으로 이동합니다.");
	    			self.close();
	    			*/
	    			//var ff = opener.document.forms["MOBILE"];
	    			//alert("감사합니다. 휴대폰인증이 완료 되었습니다.\n회원가입 창으로 이동합니다.");
					//window.opener.MOBILE.persName.value = frm.user_name.value;
					//window.opener.MOBILE.persBirth.value = frm.birth.value;
					//alert(form_value+" / " +nField+" / " + window.opener.MOBILE.persName.value);
					
					//alert(window.opener.MOBILE.persName.value+ " ? " +opener.document.getElementById("persName").value);
					//opener.document.getElementById("persName").value = "김아름";//frm.user_name.value;
					//opener.document.getElementById("persBirth").value = "19871118";//frm.birth.value;
					//alert(opener.document.getElementById("persName").value);
            		//opener.document.location.href = "/member/km_join_step2.do";
            		//self.close();
        ...
        <div align="center">
            <form name="form_auth">
                <table width="589" cellpadding="0" cellspacing="0">
                    <tr style="height:14px"><td style="background-image:url('/images/research/img/boxtop589.gif');"></td></tr>
                    <tr>
                        <td style="background-image:url('/images/research/img/boxbg589.gif')">
        
                            <!-- 상단 테이블 Start -->
                            <table width="551px" align="center" cellspacing="0" cellpadding="16">
                                <tr style="height:17px">
                                    <td style="background-image:url('/images/research/img/ttbg551.gif');border:0px " class="white">
                                        <span class="bold big">[인증요청]</span> 이 페이지는 휴대폰 인증요청 페이지입니다.
                                    </td>
                                </tr>
                                <tr>
                                    <td style="background-image:url('/images/research/img/boxbg551.gif') ;">
                                    <!-- 
                                        <p class="align_left">소스 수정 시 가맹점의 상황에 맞게 적절히 수정 적용하시길 바랍니다.</p>
                                        <p class="align_left">결제에 필요한 정보를 정확하게 입력하시어 인증를 진행하시기 바랍니다.</p>
                                         -->
                                         <p class="align_left">아래 인증 요청 버튼을 눌러 인증을 진행하시기 바랍니다.</p>                                        
                                    </td>
                                </tr>
                                <tr style="height:11px"><td style="background:url('/images/research/img/boxbtm551.gif') no-repeat;"></td></tr>
                            </table>
                            <!-- 상단 테이블 End -->
        
                            <!-- 주문 정보 출력 테이블 Start -->
                            <table width="527" align="center" cellspacing="0" cellpadding="0" class="margin_top_20" style="display: none;">
                            ...
                            <table width="527" align="center" cellspacing="0" cellpadding="0" class="margin_top_20">
                                ...
                                <tr id="show_pay_btn">
                                    <td colspan="2" align="center">
                                        <input type="image" src="/images/research/img/btn_certi.gif" onclick="return auth_type_check();" width="108" height="37" alt="결제를 요청합니다" />
                                    </td>
                                </tr>
                            </table>
                            <!-- 결제 버튼 테이블 End -->
                    ...
                    <tr><td><img src="/images/research/img/boxbtm589.gif" alt="Copyright(c) KCP Inc. All rights reserved."/></td></tr>
                </table>
                ...
                <!-- 웹사이트아이디 -->
                <input type="hidden" name="web_siteid"   value="J00290000001"/> 
                ...
                <input type="hidden" name="Ret_URL"      value="https://www.dietitian.or.kr/pages/member/kcpcert_proc_req.jsp" />
